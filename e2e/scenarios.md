...
| `clock-restore/restore-real-clock.spec.ts`              | none         | The clock project's teardown, which puts the real clock back after every clock-moving scenario and asserts, as a file running after them, that both processes report it                                                                                                                |
| `clock/clock-shift.spec.ts`                             | none         | The clock seam itself: a shift through `just e2e-clock` reported by both running processes, and the browser pinned to the same instant so later scenarios compare one clock read twice                                                                                                 |
...
| `refused-tradeoff.spec.ts`                              | none         | A tradeoff requested on a materialized, unsolved week answers 409 naming the solve it is missing and leaves the shortfall and the revision history untouched; solved, the same week offers and accepts one                                                                             |
...
| `s05-pin-and-reflow.spec.ts`                            | S5           | A discrete drag across a real day-column boundary: the block never follows the pointer, the hairline names the quarter hour and vanishes over the next column, the drop lands as one redraw, and the verdict answers in the same painted frame as the currency that says what answered |
...
| `s22-no-motion.spec.ts`                                 | S22          | Every element on eleven routes, over its computed style: nothing spins, and nothing transitions; and both amber notice surfaces, the raised panel by its surface and the promotion panel by the geometry its table composes inside the notice surface                                  |
| `s25-progress-does-not-manufacture-a-shortfall.spec.ts` | S25          | The three-pin sequence on the tight-capacity week: counted pins never move the deadline gap, unrelated pins grow it by exactly the minutes that land in the window, and a Fitness pin drops the solver's remaining floor figure while the probe's reservation stays at zero            |
...
| `s7-verdict-panel-height.spec.ts`                       | S7           | The verdict panel's fixed height across a sequence of pins: four `boundingBox()` readings over a week whose rows the pins change, equal throughout and changing in row count                                                                                                           |
