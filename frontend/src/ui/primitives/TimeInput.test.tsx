...
describe("TimeInput", () => {
  ...
  it("hears a key pressed inside the field through a forwarded key handler", () => {
    const onKeyDown = vi.fn<(event: { key: string }) => void>();
    const { container } = renderTime({ onKeyDown });

    fireEvent.keyDown(timeField(container), { key: "Escape" });
