 ...
 *
 * THE ROW CLAIMS THE KEYBOARD BY BEING FOCUSED. `x`, `Shift+X` and `m` are bare keys bound by the route,
 * and the row they act on is the one holding focus, which a reader reaches by tabbing to its controls.
 * Nothing here invents a second focus channel: the controls are real buttons and the ring is the kit's.
 *
  ...
  return (
    <>
      <LedgerRow
        ...
        area={area}
      >
        {/* Focus reaches this span from the buttons inside it, which is how the row becomes the one a
            bare keystroke acts on. */}
        <span className="flex items-center gap-2" onFocus={() => actions.onEnter(row)}>
          <span className="text-eyebrow text-text-muted">{stateReading(row, section, zone)}</span>
          {form === null ? <OutcomeControls row={row} actions={actions} /> : null}
          {form?.kind === "partial" ? (
