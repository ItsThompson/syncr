 ...
 *
 * THE GRID, THE PIN PATH, THE VERDICT PANEL AND THE APPROVE ACTION ARE THE SCREEN'S OWN, reused rather than restated.
 * Editing inside the session pins through the same write, the solver reflows the remainder, and the verdict updates
 * live because the panel is drawn from the same reading the screen draws it from. A second panel fed by this mode's own
 * payload would disagree with the screen's the moment a solve landed between two reads, and a second approve control
 * would be a second way to commit a week.
 *
...
import { RaisedPanel, SummaryStrip, VerdictPanel, WeekGrid } from "../../../ui/domain";
import { columnLabel, weekRange } from "../labels";
...
export function SessionMode({
  ...
  return (
    <section aria-label="Weekly session">
      <SessionHeader isoWeek={isoWeek} range={weekRange(dates)} />
      <div className="flex flex-col gap-3.25 px-3.75 py-3.25">
        {session.statement === null ? null : (
          <p className="text-sm text-text-muted">{session.statement}</p>
        )}
