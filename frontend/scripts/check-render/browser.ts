...
/**
 * One page, rendered and screenshotted, as the PNG bytes.
 *
 * `--force-device-scale-factor=1` so a pixel in the answer is a CSS pixel wherever this runs, and
 * `--hide-scrollbars` so a scrollbar cannot shift the geometry being measured. The virtual time budget is what
 * makes the shot deterministic: it is taken after the page has settled rather than after a wall-clock guess.
 */
export async function screenshot(request: ShotRequest): Promise<Buffer> {
  ...
  await run(request.browser, [
    "--headless=new",
    "--disable-gpu",
    "--allow-file-access-from-files",
    "--hide-scrollbars",
    "--force-device-scale-factor=1",
    `--window-size=${request.widthPx},${request.heightPx}`,
    `--screenshot=${shot}`,
