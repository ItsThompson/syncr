...
## The version is passed in rather than read here

Its caller reports the same figure on the same response. Two reads of the counter inside one
request can straddle a bump under ``READ COMMITTED``, and the pair would then disagree: the view
would name one input state and the verdict beside it another.

## The cost, and which branch pays it

The slot branch is one indexed select. The fallback is a whole assembly, which is eighteen
...
pays is the one whose week has no current proposal -- which is every week between an approval and
the next solve.
"""
...
class ServedVerdict:
    ...
    async def for_week(self, iso_week: IsoWeek, *, now: datetime, input_version: int) -> Verdict:
        ...
        verdict names.
        """
        held = await self._proposals.find(iso_week)
        if held is not None and held.input_version == input_version:
            return verdict_of(held.verdict)
        return self._probe.offered_verdict_for(
            await self._assembler.assemble(iso_week, now)
        ).verdict
...
class CurrentWeekVerdict:
    ...
    **Which week that is, is resolved in the HOME zone**, from the same reading every backlog-wide
    ...
    async def read(self) -> Verdict:
        """The current week's verdict. Writes nothing, and needs no solve to have completed."""
        now = self._clock()
        ...
        return await self._served.for_week(
            week, now=now, input_version=await self._versions.tracked_version(week)
        )
