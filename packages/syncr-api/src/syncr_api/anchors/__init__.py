...
it. **Those four are the only modules here that compute time nobody stored.** The two others that
name an interval at all merely rebuild one they were handed: ``queries.py`` from the bounds a
request asked to read, and ``repository.py`` from the two columns a row holds. A test enumerates the
six against the source, because the claim is about every other module in the package and a negative
one is not worth stating unless it is checked.
