...
copy of the full declaration fails this walk the moment it lands. One stated edge: a body
that aliases its imports (``DayTypeRepository as DT``) evades the reading, whose names come
from ``ast.Call`` nodes only; a pasted copy carries the canonical names, which is the
duplication this tripwire exists to catch.
