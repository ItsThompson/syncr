    ...
    *,
    ...
    while improving and iterations < budget.move_evaluations:
        ...
        for move in moves(current, preferences):
            iterations += 1
            if _stop(iterations, budget=budget, cancelled=cancelled):
                return Improved(
                    attempt=current,
                    breakdown=breakdown,
...
def _stop(iterations: int, *, budget: SolveBudget, cancelled: Cancelled) -> bool:
    """Whether the budget is spent, or the caller has stopped wanting the result.

    The two are one question because a caller reads one answer: the search stops and returns the
    plan it had. Which of the two stopped it is not a fact the plan carries, because a plan produced
    under a smaller budget is a legal plan and not a partial one.
    """
