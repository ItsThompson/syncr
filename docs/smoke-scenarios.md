...
run. It reads all four of the table's columns: every automated row must have a test naming its scenario
and must name the spec file that test lives in, every scenario a test names must be marked automated here,
and every repository path and `just` recipe cited in either the Where column or the observation cell must
resolve. The observation cell is prose, but a backticked path or recipe inside it is a citation a reader
follows, so those are bounded even though the prose around them is not. Its first version read two columns
and its first run found four disagreements, one of them a row claiming an assertion no file in the suite
made.
...
|---|---|---|
| `reference_week` | `just seed-reference` | A frame span crossing midnight, a fifteen-minute compact block, an interview anchor with prep, transit and a recovery window, an anchor conflict, a queue binding, four Areas with floors, three tasks, two of them with deadlines, and two anchor types over three mocked feeds |
...
| `maturity_corpus` | `just seed-maturity-corpus` | Outcomes recorded on every block the weeks in the horizon have ended, and a print of what it reached. Every week solves; any failure stops the seed |
...
| S5 | Pin and reflow: the block does not follow the cursor, one redraw on drop, the verdict on the same redraw | automated | `s05-pin-and-reflow.spec.ts`: a real `setPointerCapture` drag lifts the solved `tight_capacity` week's earliest deadline chunk across a real day-column boundary and back, and asserts the four observations as painted states: the block's box never moves while the pointer does, the one hairline names a quarter hour at the scale the grid draws at and vanishes entirely over the next column, the release lands as one mutation batch and one painted frame (marker gone, block at the stated quarter, glyph together), and the verdict words change in the same painted frame as the `solving` currency, before the worker settles. Shown to fail with the pressed block displaced under the pointer, with the marker naming an off-quarter minute, with the drag state clearing a tick after the drop applies, and with the response's verdict dropped |
...
| S17 | An unfillable slot's label opens prefilled capture, producing a soft preference and no pin | automated, and the capture flow **currently red by declaration** | `s01-materialization.spec.ts` asserts the `no_fitting_content` rendering: a solved week's Fitness slot declares sixty minutes and the Gym habit holds forty-five, so the Area has content but none of it can take the slot's duration, and each unfillable slot stays at its declared time and names its Area. `s17-capture-from-an-unfillable-slot.spec.ts` drives the flow behind the label: its first case asserts that a label the grid draws as a control takes the pointer while the band holding it still refuses one; its second drives activation, the prefill and the confirm, and is marked as expected to fail, so the day the flow exists the suite goes red for passing unexpectedly and the marker has to be removed. Three things it waits on, each measured: the label an empty slot's band does not carry (ticket 1350), the reader of the capture URL and the task-addressed preference write (ticket 1490). The fourth it used to name, a grid canvas that grew until layout stopped, is fixed: the grid now measures the space the display offers it rather than the element its own output sizes, so the surface holds still and presses land where they aim |
...
| S21 | Keyboard only, including a sliver-tier block | partly automated | `s21-keyboard-and-focus.spec.ts` drives the whole session on the keyboard, and every step is asserted as a TRANSITION rather than as a presence: from each shell-mounted route, `Tab` focuses the skip link, the live DOM count derives the eight focus stops it bypasses before `main`, and `Enter` moves `document.activeElement` inside that landmark; `g w` by URL, `j` by the tier focus lands on -- asserted to land ON a sliver-tier block at the zoom that draws one -- `Shift+Down` by the pin read back over the api, `n` by the value the caret typed, `c` by the confirm request the key sends and by `confirmedAt` moving off null, `Shift+A` by the approval request it sends and the answer it got. Each key was shown to fail with its own press removed. The session runs at the 1440x900 display whose arithmetic `tokens.css` documents: at the suite's default viewport the grid measures too shallow for any tier below compact to render at all |
...
| S29 | The calendar does not go blank at the week boundary | automated | `clock/s29-sunday-to-monday-boundary.spec.ts`: the clock crosses a Sunday-to-Monday boundary through `just e2e-clock`, the newly covered week acquires a plan with no user action, and a frame span crossing the boundary is emitted exactly once across the two-week horizon, asserted as a count |
...
| S35 | Verdict transitions recorded exactly once, reads write nothing, the ratio is a number | automated | `s30-s35-approval-and-verdict-events.spec.ts`. Two reads and a no-op maintainer tick append nothing; no two consecutive rows agree on both the reading and its provenance; the case arranges two episodes on one week, with one opening row session-flagged and the other unflagged, then asserts the product's own `caught_early_over` ratio is exactly 0.5 |
| S36 | The verdict cannot be gamed and it notices the clock | automated | `s36-verdict-and-the-clock.spec.ts` drives the two halves that need no clock MOVED, on `owes_more_than_a_week`: a verdict read on a week the clock has reached counts capacity from the reading instant onward, asserted as a figure against the same demand on the week ahead of it, which counts its whole span; and the denominator keeps the whole week in both. A skip then raises the gap by exactly the minutes the block held and returns none of its span to capacity. `clock/s36-deadline-past-now.spec.ts` drives the third observation: the stack clock is shifted past the deadline the fixture seeded in the future, and the whole remaining estimate is reported as the shortfall, asserted as a figure. The case states in its own comment why a deadline seeded in the past is not this case: it was never inside the horizon's capacity, so nothing is observed about the transition |
...
the `no_fitting_content` case above is S17's reason half; and the refused tradeoff is the bound that
