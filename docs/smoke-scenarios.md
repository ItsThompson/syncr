...
|---|---|---|
| S5, S7 | Each needs a driven interaction on the week grid: a discrete drag with a real `setPointerCapture`, and a panel height measured across a sequence of pins. The harness has the browser and the credential; what is missing is the per-screen driving. S7 also has to account for a measured product behaviour before it reads a row count, and it is not per pin: a pin answers a verdict that enumerates no concessions, and the re-solve the pin schedules leaves a pending proposal about five seconds later whose stored verdict every later read serves, which enumerates none either. So on a week with a shortfall the panel loses its offer rows and gains the no-remedy statement at the FIRST pin and keeps them lost while that proposal stands, rather than recovering between pins | ticket 1572 |
