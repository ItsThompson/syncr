...
**A value used by more than one sheet is a token, without exception.** Five values were found re-derived across sheets during the build and promoted: the overlay scrim, the hatch's lightening step, the verdict panel's fixed height, the reason record's clause-label column, and the stacking order. The clause-label column had independently acquired three different widths in three sheets, which is exactly the drift the inherited language warned about when its own two references disagreed on a value the tokens were silent about.
